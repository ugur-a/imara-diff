...
pub struct SimpleLineDiff<'a, T: EndsWithNewline + ?Sized + Hash + Eq + Display>(
    pub &'a Interner<&'a T>,
);

impl<T: EndsWithNewline + Hash + Eq + Display + ?Sized> UnifiedDiffPrinter for SimpleLineDiff<'_, T> {
    fn display_header(
        &self,
